...
- content_hash = blake2b (digest 32 -> 64 hex) of summary+body (blake3 dep via liteparse, blake2b is stdlib near-equivalent)
...
def _frontmatter_yaml(payload: UnitPayload, content_hash: str, ingested_at: str) -> str:
    ...
    # Ensure values are quoted if they contain special chars
    def q(v: str | None) -> str:
        if v is None or v == "":
            return '""'
        # If contains colon+space or leading special, quote
...
def write_unit(payload: UnitPayload, corpus_root: Path) -> Path:
    ...
    # Compute content_hash over summary + body_markdown (blake2b 32 -> 64 hex, blake3-like)
    ...
    # Idempotent: if file exists and hash matches, skip
    if out_path.exists():
        existing = out_path.read_text(encoding="utf-8")
        m = re.search(r"content_hash:\s*([a-f0-9]+)", existing)
        if m and m.group(1) == content_hash:
            # Also check body equality to avoid false positive on hash collision
            # If hash matches, skip rewrite to preserve mtime
            if existing == full or existing.split("ingested_at:")[0] == full.split("ingested_at:")[0]:
                # Compare without ingested_at (which changes every run) — check summary+body part
                ...
                try:
                    ...
                    pass
            # Hash matches but ingested_at would differ — still skip to keep idempotent per test expectation
            # Verify via read after second write in test: they compare read_text equality after second write
            # Our second write would update ingested_at, breaking idempotency test.
            # So for idempotent case, return without rewriting when hash matches and body same (ignore ingested_at)
            # We already returned above for body same; else fall through to rewrite
            # But test expects second write not to change file when same payload — so we must skip rewrite entirely when hash matches
            return out_path
